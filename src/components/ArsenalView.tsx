    ...
    <div className="pb-20">
      <AnimatePresence mode="wait">
        {!selectedSlug ? (
          ...
          >
            {/* The Hero Header */}
            <Card className="border-none shadow-[0_8px_30px_rgb(0,0,0,0.06)] dark:shadow-[0_8px_30px_rgb(0,0,0,0.4)] rounded-[2.5rem] bg-indigo-950 text-white overflow-hidden relative pb-4">
              <div className="absolute top-0 right-0 p-8 opacity-10 pointer-events-none">
                <Sword className="w-64 h-64" />
              </div>
              <CardContent className="p-8 md:p-12 relative z-10 flex flex-col lg:flex-row items-center justify-between gap-10">
                
                {/* Title & Subtext */}
                <div className="space-y-4 flex-1 w-full text-center lg:text-left">
                  <h1 className="text-4xl md:text-5xl lg:text-6xl font-black tracking-tight">The Arsenal</h1>
                  <p className="text-lg md:text-xl text-indigo-200 font-medium max-w-2xl">
            ...
            <div className="flex justify-center md:justify-start">
              <div className="flex gap-2 p-1.5 bg-slate-100 dark:bg-zinc-900 rounded-full w-fit border border-slate-200 dark:border-zinc-800 shadow-sm">
                <button 
                  onClick={() => setActiveTab('core')}
                  className={`px-6 py-2.5 rounded-full text-sm font-black tracking-tight transition-all ${activeTab === 'core' ? 'bg-white dark:bg-zinc-800 shadow-sm text-indigo-700 dark:text-indigo-400' : 'text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200'}`}
                >
                  Core Intelligence
                </button>
                <button 
                  onClick={() => setActiveTab('tools')}
                  className={`px-6 py-2.5 rounded-full text-sm font-black tracking-tight transition-all ${activeTab === 'tools' ? 'bg-white dark:bg-zinc-800 shadow-sm text-emerald-600 dark:text-emerald-400' : 'text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200'}`}
                >
                  The Tool Shed
                </button>
              </div>
            ...
            {activeTab === 'core' ? (
              <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
                ...
                <Card className="border border-indigo-500/20 shadow-xl shadow-indigo-500/5 rounded-[2.5rem] bg-gradient-to-r from-indigo-500/10 to-emerald-500/10 dark:from-indigo-500/20 dark:to-emerald-500/20 backdrop-blur-md overflow-hidden relative group">
                  <CardContent className="p-8 md:p-10 relative z-10 flex flex-col md:flex-row items-center justify-between gap-8">
                    <div className="space-y-4 flex-1">
                      <div className="flex items-center gap-2 text-indigo-700 dark:text-indigo-400">
                        ...
                        <h2 className="text-sm font-black uppercase tracking-widest">Stocrates Recommends</h2>
                      </div>
                      <h3 className="text-2xl md:text-3xl font-black tracking-tighter text-slate-900 dark:text-white">
                    ...
                    <div className="shrink-0 w-full md:w-auto">
                      <Button 
                        ...
                        className="w-full md:w-auto h-14 rounded-2xl bg-indigo-600 hover:bg-indigo-700 text-white font-bold px-8 shadow-[0_0_30px_-5px_rgba(79,70,229,0.5)] transition-all group-hover:scale-105"
                      >
                ...
                <div className="space-y-6">
                  <h2 className="text-xl font-bold tracking-tight text-slate-900 dark:text-white px-2">
                    Core Curriculum
                  </h2>
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {modules.map((mod, i) => (
                      ...
                      >
                        <Card className={`h-full border ${mod.locked ? 'border-slate-200 dark:border-zinc-800 bg-slate-50/50 dark:bg-zinc-900/50 opacity-80 filter grayscale-[0.2]' : 'border-slate-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 hover:border-indigo-300 dark:hover:border-indigo-500/50 hover:shadow-xl transition-all duration-500'} rounded-3xl overflow-hidden`}>
                          <CardContent className="p-8 space-y-6 relative h-full flex flex-col justify-between">
                            <div className="space-y-4">
                              <div className="flex items-start justify-between">
                                <Badge variant="outline" className={`text-[9px] font-black uppercase tracking-widest px-2 py-1 rounded-lg ${mod.locked ? 'text-slate-500 border-slate-300' : 'text-blue-600 border-blue-200 bg-blue-50 dark:text-blue-400 dark:border-blue-900/50 dark:bg-blue-900/20'}`}>
                                  {mod.category}
                                </Badge>
                            ...
                            {!mod.locked && (
                              <div className="pt-4 border-t border-slate-100 dark:border-zinc-800">
                                <span className="text-xs font-bold text-indigo-600 dark:text-indigo-400 flex items-center gap-1 group-hover:translate-x-1 transition-transform cursor-pointer">
                                  {mod.progress === 100 ? 'Review Module' : 'Continue Module'} <ArrowRight className="h-3 w-3" />
                                </span>
                              </div>
                            )}
                            {mod.locked && (
                              <div className="pt-4 border-t border-slate-200 dark:border-zinc-800">
                                <span className="text-[10px] font-black uppercase tracking-widest text-slate-400 flex items-center gap-2">
                                  <Lock className="h-3 w-3" /> Unlocks at Level 3
                                </span>
                              </div>
                            )}
                          </CardContent>
          ...
          >
            ...
            <div className="max-w-7xl mx-auto mb-8">
              ...
              >
                ...
                <span className="font-bold text-sm tracking-tight text-slate-700 dark:text-slate-300 group-hover:text-indigo-600">Back to {activeTab === 'core' ? 'Core Intelligence' : 'The Arsenal'}</span>
              </Button>
            </div>
                ...
                <div className="w-full lg:w-3/5 space-y-12 shrink-0">
                  <header className="space-y-6 text-left">
                    <div className="flex items-center gap-4">
                      <Badge className="w-fit rounded-full px-4 py-1.5 border-indigo-200 text-indigo-700 font-black uppercase tracking-widest text-[9px] bg-indigo-50 dark:border-indigo-900/50 dark:bg-indigo-900/20 dark:text-indigo-400">{selectedArticle?.category}</Badge>
                      <span className="text-[10px] font-black uppercase tracking-widest text-slate-500">{selectedArticle?.readTime} Session</span>
                    </div>
                    <h1 className="text-4xl md:text-5xl font-black leading-[1.1] tracking-tighter text-slate-900 dark:text-white text-balance">
                      ...
                      if (section.type === 'header') {
                        ...
                        return (
                          <motion.div
                            ...
                            transition={{ duration: 0.6, ease: [0.22, 1, 0.36, 1] }}
                          >
                            <HeaderTag className="text-slate-900 dark:text-white font-black tracking-tight">{section.text}</HeaderTag>
                          </motion.div>
                        );
                      }
                      ...
                      if (section.type === 'blockquote') {
                        return (
                          <motion.blockquote
                            ...
                            viewport={{ once: true, margin: "-50px" }}
                            transition={{ duration: 0.6, ease: [0.22, 1, 0.36, 1] }}
                            className="border-l-4 border-l-indigo-600 bg-indigo-50 dark:bg-indigo-900/10 p-6 rounded-r-2xl italic my-8"
                          >
                      ...
                      if (section.type === 'takeaways') {
                        return (
                          <motion.div
                            ...
                            viewport={{ once: true, margin: "-50px" }}
                            transition={{ duration: 0.6, ease: [0.22, 1, 0.36, 1] }}
                            className="bg-emerald-50 dark:bg-emerald-900/10 border border-emerald-200/50 dark:border-emerald-900/50 p-8 rounded-3xl my-10 shadow-sm"
                          >
                            <h3 className="text-xl font-bold text-emerald-900 dark:text-emerald-400 mb-6 flex items-center gap-3 m-0">
                              <Sparkles className="h-5 w-5" />
                              Key Takeaways
                            </h3>
                            <ul className="space-y-3 m-0 list-none pl-0">
                              {section.items.map((item, idn) => (
                                <li key={idn} className="flex gap-3 text-slate-700 dark:text-slate-300 items-start">
